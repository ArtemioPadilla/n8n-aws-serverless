...
class TestStackDeployment:
    ...
    def test_full_stack_deployment(self, config_loader):
        ...
        storage_stack = StorageStack(
            app, f"test-storage", config=config, environment=environment,
            network_stack=network_stack, env=env
        )
        
        database_stack = DatabaseStack(
            app, f"test-database", config=config, environment=environment,
            network_stack=network_stack, env=env
        )
        
        compute_stack = ComputeStack(
            app, f"test-compute", config=config, environment=environment,
        ...
        access_stack = AccessStack(
            app, f"test-access", config=config, environment=environment,
            compute_stack=compute_stack, env=env
        )
        
        monitoring_stack = MonitoringStack(
            app, f"test-monitoring", config=config, environment=environment,
            compute_stack=compute_stack, storage_stack=storage_stack,
    ...
    def test_environment_specific_configuration(self, config_loader):
        ...
        for environment in environments:
            ...
            network_stack = NetworkStack(
                app, f"{environment}-network", config=config, environment=environment, env=env
            )
            
            storage_stack = StorageStack(
                app, f"{environment}-storage", config=config, environment=environment,
                network_stack=network_stack, env=env
            )
            
            compute_stack = ComputeStack(
                app, f"{environment}-compute", config=config, environment=environment,
                network_stack=network_stack, storage_stack=storage_stack, env=env
            )
    ...
    def test_cross_region_deployment(self, config_loader):
        ...
        for region in regions:
            ...
            network_stack = NetworkStack(
                app, f"network-{region}", config=config, environment=environment, env=env
            )

...
class TestSecretsScanning:
    ...
    def test_no_private_keys_in_repository(self, project_root):
        ...
        text_files = list(project_root.rglob("*.py")) + \
                     list(project_root.rglob("*.yaml")) + \
                     list(project_root.rglob("*.yml")) + \
    ...
    def test_env_files_are_examples(self, project_root):
        ...
        for env_file in env_files:
            ...
            assert env_file.name == ".env.example", \
                ...
                for pattern in db_patterns:
                    ...
                    for match in matches:
                        ...
                        if not any(placeholder in match.lower() for placeholder in 
                                 ['example', 'your', 'xxx', 'placeholder', 'test']):
    ...
    def test_config_files_no_secrets(self, project_root):
        ...
        config_files = list(project_root.rglob("system.yaml")) + \
                        ...
                        for k, v in d.items():
                            ...
                            if any(secret in k.lower() for secret in secret_fields):
                                if isinstance(v, str) and v and \
                                   not any(placeholder in v.lower() for placeholder in 
                                         ['your', 'xxx', 'example', 'placeholder']):
                                    assert False, f"Potential secret in {config_file} at {current_path}: {v}"
    ...
    def test_git_secrets_config_exists(self, project_root):
        ...
        assert gitsecrets_file.exists() or pre_commit_file.exists(), \
        ...
        if pre_commit_file.exists():
            ...
            has_secrets_scanning = any(
                'detect-secrets' in str(repo) or 
                'git-secrets' in str(repo) or
                'truffleHog' in str(repo)
                for repo in repos
            )
            
            assert has_secrets_scanning, \
    ...
    def test_docker_files_no_secrets(self, project_root):
        ...
        docker_files = list(project_root.rglob("Dockerfile*")) + \
                      list(project_root.rglob("docker-compose*.yml"))
    ...
    def test_terraform_files_no_secrets(self, project_root):
        ...
        tf_files = list(project_root.rglob("*.tf")) + \
                  list(project_root.rglob("*.tfvars"))

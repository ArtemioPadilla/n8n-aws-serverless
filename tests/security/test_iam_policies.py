...
class TestIAMPolicies:
    ...
    def test_task_role_least_privilege(self, app, test_config):
        ...
        network_stack = NetworkStack(
            app, "test-network", config=test_config, environment="test",
            env=Environment(account="123456789012", region="us-east-1")
        )
        
        storage_stack = StorageStack(
            app, "test-storage", config=test_config, environment="test",
            network_stack=network_stack,
            env=Environment(account="123456789012", region="us-east-1")
        )
        
        compute_stack = ComputeStack(
            app, "test-compute", config=test_config, environment="test",
            network_stack=network_stack, storage_stack=storage_stack,
            env=Environment(account="123456789012", region="us-east-1")
        )
        ...
        task_roles = {k: v for k, v in roles.items() 
                     if "TaskRole" in k or "taskRole" in str(v)}
                    ...
                    if isinstance(resources, list):
                        for resource in resources:
                            if resource == "*":
                                ...
                                assert any(action in ["logs:CreateLogGroup", "logs:CreateLogStream"] 
    ...
    def test_no_admin_policies(self, app, test_config):
        ...
        for role_name, role in roles.items():
            ...
            for policy_arn in managed_policies:
                assert "AdministratorAccess" not in str(policy_arn), \
                    f"Admin policy found in {role_name}"
                assert "PowerUserAccess" not in str(policy_arn), \
    ...
    def test_secrets_access_restricted(self, app, test_config):
        ...
        for secret_name, secret in secrets.items():
            ...
            assert secret.get("Properties", {}).get("Description"), \
    ...
    def test_network_security_groups(self, app, test_config):
        ...
        for rule_name, rule in sg_rules.items():
            ...
            if cidr == "0.0.0.0/0":
                ...
                assert from_port in allowed_internet_ports, \
    ...
    def test_encryption_at_rest(self, app, test_config):
        ...
        for fs_name, fs in efs_systems.items():
            assert fs.get("Properties", {}).get("Encrypted") is True, \
        ...
        for db_name, db in rds_instances.items():
            assert db.get("Properties", {}).get("StorageEncrypted") is True, \
        ...
        for file_path in python_files:
            ...
            for pattern in secret_patterns:
                ...
                for match in matches:
                    ...
                    if any(exception in match.lower() for exception in 
    ...
    def test_vpc_endpoints_for_aws_services(self, app, test_config):
        ...
        network_stack = NetworkStack(
            app, "test-network", config=test_config, environment="production",
            env=Environment(account="123456789012", region="us-east-1")
        )

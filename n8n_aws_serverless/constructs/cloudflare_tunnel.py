    ...
    def __init__(
        ...
        *,
        ...
        self.container = task_definition.add_container(
            ...
            secrets={
                "TUNNEL_TOKEN": ecs.Secret.from_secrets_manager(tunnel_secret)
            },
            logging=ecs.LogDriver.aws_logs(
                stream_prefix="cloudflare",
                log_group=log_group
            ),
            health_check=ecs.HealthCheck(
                command=["CMD-SHELL", "wget -q -O /dev/null http://localhost:2000/ready || exit 1"],
